...
strm = OpenFortran["born_"<>name<>".mf"];
WriteStringn[strm, "subroutine born_"<>name<>"(p,b)"];
...
WriteStringn[strm, "double precision b"]
...
WriteStringn[strm, ""]
defvars=Map[Join[#,{TAG}]&,Subsetn[GetVariables[optimizedRules]/.indices,4]];
...
WriteStringn[strm, ""]
vars=GetVariables[optimizedRules];
...
AssignValues[ulist,GetValues[optimizedRules]/.functions];
bornout=born/.functions;
Splice["born_"<>name<>".mf", PageWidth -> 72];
(*FileTemplateApply[FileTemplate["born_"<>name<>".mf"],"born_"<>name<>".f"]*)
...
WriteStringn[strm, "gsed -i -e \"s/List\(//g\" "<>"born_"<>name<>".f"];
WriteStringn[strm, "gsed -i -e \"s/,TAG\)//g\" "<>"born_"<>name<>".f"];
WriteStringn[strm, "gsed -i -e 's/\\t/      /g' "<>"born_"<>name<>".f"];
WriteStringn[strm, "mv born_"<>name<>".f " <>"born_"<>name<>".F"];
Close[strm];
<<"!chmod +x finalize.sh"
<<"!./finalize.sh"
<<"!rm *.f-e"
